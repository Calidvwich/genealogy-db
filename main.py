...
--
-- 或使用 pg_restore / psql 管道方式：
-- psql -U postgres -d genealogy_db -c "\COPY members(...) FROM 'members_load.csv' WITH (FORMAT CSV, NULL '');"
...
-- 单身定义：该成员在所有子女记录中，其配偶位置为 NULL
--           即：没有任何子女的 father_id/mother_id 同时指向另一人
